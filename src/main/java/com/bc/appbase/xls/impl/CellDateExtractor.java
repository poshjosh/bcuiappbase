/*
 * Copyright 2017 NUROX Ltd.
 *
 * Licensed under the NUROX Ltd Software License (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.looseboxes.com/legal/licenses/software.html
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package com.bc.appbase.xls.impl;

import com.bc.appcore.util.TextHandler;
import java.util.Date;
import java.util.Objects;
import java.util.function.BiConsumer;
import jxl.Cell;
import jxl.write.biff.DateRecord;

/**
 * @author Chinomso Bassey Ikwuagwu on May 9, 2017 2:18:59 PM
 */
public class CellDateExtractor extends AbstractProcessor<Cell, Date> {

    private final TextHandler textHandler;
    
    public CellDateExtractor(TextHandler textHandler, 
            Date outputIfNoResult, BiConsumer<Cell, Exception> errorHandler) {
        super(outputIfNoResult, errorHandler);
        this.textHandler = Objects.requireNonNull(textHandler);
    }

    @Override
    public Date process(Cell cell) {
        
        return this.getDate(cell);
    }

    public Date getDate(Cell cell) {
        
        final Date date;
        
        if(cell instanceof DateRecord) {
            date = ((DateRecord)cell).getDate();
        }else{
            // Eg:  22 Dec 16 (RPB NLT 23 Dec 16)
            //
            final String cellContents = cell.getContents();
            final String trackStartStr = this.textHandler.isNullOrEmpty(cellContents) ? null : this.textHandler.getLastDateStr(cellContents);
            date = this.textHandler.getDate(trackStartStr);
        }
        
        return date;
    }
}

