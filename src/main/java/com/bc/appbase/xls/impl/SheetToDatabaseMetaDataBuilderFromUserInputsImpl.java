...
import com.bc.appbase.ui.actions.PromptSelectEntityType;
...
public class SheetToDatabaseMetaDataBuilderFromUserInputsImpl 
    ...
    public SheetToDatabaseMetaDataBuilderFromUserInputsImpl(
    ...
    public SheetToDatabaseMetaData build(Sheet sheet, SheetToDatabaseMetaData outputIfNone) {
        ...
        if(entityType == null) {
            ...
            final int headerRowOffset = this.promptSelectRowOffset(sheet, entityType, 
                    "Select row containing Headers. Default is: " + 0, 0);

            final int [] columnIndexes = this.promptSelectColumns(sheet, entityType, 
                    "Select columns to extract data from. Default is all columns", null);

            final SheetProcessorContext context = objectFactory.getOrException(SheetProcessorContext.class);
            ...
            final SheetToDatabaseData data = 
                    prompt.execute(appContext, objectFactory, uiContext, entityType, sheet, headerRowOffset, columnIndexes);

            final int dataRowOffset = this.promptSelectRowOffset(sheet, entityType, 
                    "Select first row to extract. Default is: " + headerRowOffset + 1, headerRowOffset + 1);

            output = new SheetToDatabaseMetaDataImpl(data, objectFactory.getOrException(TypeProvider.class),
    ...
    @Override
    public int promptSelectRowOffset(Sheet sheet, Class entityType, 
        ...
        uiContext.updateTableUI(table, entityType, -1);
        
        this.showDialog(table, title, "Select Row");
    ...
    @Override
    public int [] promptSelectColumns(Sheet sheet, Class entityType, 
            String title, int [] outputIfNone) {
        ...
        this.showDialog(table, title, "Select Columns");
    ...
    public void showDialog(Component component, String description, String title) {
    ...
    public Class promptSelectEntityType(Set<Class> options) {
        return new PromptSelectEntityType().execute(appContext, objectFactory, uiContext, 
                Collections.singletonMap(ParamNames.ENTITY_TYPE+"List", options));
    }
    
    public Set<Class> getOptions(Predicate<Class> test) {
        final Set<Class> entityTypes = this.appContext.getActivePersistenceUnitContext().getMetaData().getEntityClasses();
