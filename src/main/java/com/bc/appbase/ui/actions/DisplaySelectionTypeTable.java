...
public class DisplaySelectionTypeTable implements Action<App, Object> {
    ...
    public Object execute(App app, Map<String, Object> params) 
        ...
        final SearchResultsPanel resultsPanel = frame.getSearchResultsPanel();
        final String panelName = this.getClass().getName()+'_'+resultsPanel.getClass().getSimpleName(); 
        resultsPanel.setName(panelName);
        ...
        resultsPanel.load(app.getSearchContext(entityType), searchResults, app.getRandomId());
        ...
        app.getSerialColumnName();
        
        final int serialColumnIndex = this.getSerialColumnIndex(app, table.getModel(), 0);
    ...
    public int getSerialColumnIndex(App app, TableModel model, int outputIfNone) {
        if(model instanceof EntityTableModel) {
            return ((EntityTableModel)model).getResultModel().getSerialColumnIndex();
        }else{
            final String serColName = app.getSerialColumnName();
            final int colCount = model.getColumnCount();
            for(int col=0; col<colCount; col++) {
                if(Objects.equals(serColName, model.getColumnName(col))) {
                    return col;
                }
            }
            return outputIfNone;
