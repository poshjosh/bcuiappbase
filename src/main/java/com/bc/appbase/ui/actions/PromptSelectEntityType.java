...
public class PromptSelectEntityType implements Action<App, Class> {
    ...
    public Class execute(App app, Map<String, Object> params) {
        ...
        return this.execute(app, app, app.getUIContext(), params);
    }
    ...
    public Class execute(
        ...
        final Set<Class> classSet = this.getOptions(appContext, params);
        
        final ParameterExtractor parameterExtractor = objectFactory.getOrException(ParameterExtractor.class);
        
        final Predicate<Class> filter = parameterExtractor
                .getFirstValue(params, Predicate.class, (cls) -> true);
        
        return this.execute(uiContext, classSet, filter);
    }

    public Class execute(UIContext uiContext, Set<Class> classSet, Predicate<Class> filter) {
    ...
    public Set<Class> getOptions(AppContext appContext, Map<String, Object> params) {
        final Set<Class> output;
        final Collection<Class> optionsParam = (Collection<Class>)params.get(ParamNames.ENTITY_TYPE+"List");
        if(optionsParam == null) {
            
            output = appContext.getActivePersistenceUnitContext().getMetaData().getEntityClasses();
