/*
 * Copyright 2017 NUROX Ltd.
 *
 * Licensed under the NUROX Ltd Software License (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.looseboxes.com/legal/licenses/software.html
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package com.bc.appbase.ui;

import com.bc.appbase.App;
import com.bc.appcore.jpa.SelectionContext;
import java.awt.Component;
import java.awt.Font;
import java.util.Date;

/**
 * @author Chinomso Bassey Ikwuagwu on Apr 6, 2017 7:40:50 PM
 */
public class DtbTableCellComponentModel extends ComponentModelImpl {

    public DtbTableCellComponentModel(App app) {
        super(app);
    }

    public DtbTableCellComponentModel(
            App app,
            SelectionContext selectionContext, 
            DateFromUIBuilder dateFromUIBuilder, 
            DateUIUpdater dateUIUpdater) {
        super(app, selectionContext, dateFromUIBuilder, dateUIUpdater);
    }

    public DtbTableCellComponentModel(
            App app, SelectionContext selectionContext, 
            DateFromUIBuilder dateFromUIBuilder, DateUIUpdater dateUIUpdater, 
            Font font, int width, int height) {
        super(app, selectionContext, dateFromUIBuilder, dateUIUpdater, font, width, height, -1);
    }
    
    @Override
    public Component getComponent(Class valueType, String name, Object value) {
        final Component component;
        if(this.getSubClassTest().apply(valueType, Date.class)) {
            component = this.getTextComponent(valueType, name, value);
        }else {
            component = super.getComponent(valueType, name, value);
        }
        return component;
    }    

    @Override
    public Component getTextComponent(Class valueType, String name, Object value) {
        return this.getTextArea(valueType, name, value);
    }
}
